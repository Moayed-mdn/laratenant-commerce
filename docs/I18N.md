...
# 58. Multilingual Routing

## Rule

All links MUST include the locale prefix. The app uses `[locale]` in the URL
structure (e.g., `/en/stores/1/dashboard` or `/ar/stores/1/dashboard`).

## Client Components

Use the `useLocale()` hook from `next-intl`:
...
<Link href={ROUTES.store(locale, storeId).dashboard()}>Dashboard</Link>
```

## Server Components

Receive `locale` as a prop from the parent page:
...
export default async function MyComponent({ storeId, locale }: Props) {
  return <Link href={ROUTES.store(locale, storeId).dashboard()}>Dashboard</Link>;
}
```
...
router.push(ROUTES.store(locale, storeId).dashboard());
```
